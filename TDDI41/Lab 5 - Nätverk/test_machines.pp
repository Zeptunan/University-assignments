...
class TestNätverksMaskiner(unittest.TestCase):
    ...
    def test_IP(self):
        self.assertEqual(self.run_command('cat /etc/network/interfaces | grep 10.0.0.4'), 0) #ändrar ip baserat på maskinen jag testar
    ...
    def test_hostname(self):
        self.assertEqual(self.run_command('cat /etc/hostname | grep client-2'), 0) #ändrar även namnet baserat på maskinen
